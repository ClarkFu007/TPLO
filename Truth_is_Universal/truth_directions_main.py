...
def run_step1(model_family, model_size, model_type):
    ...
    1-32 for all four datasets to run this cell. (Figure 2 in the paper).
    ...
    """
    if model_size == '13B':
        layers = np.arange(1, 41, 1)
    else:
        layers = np.arange(1, 33, 1)
    """
    ...
    for dataset in datasets_separation:
        ...
        for layer_nr in layers:
            ...
            layer_nr = layer_nr - 1  # from 0 to 31
            dm = DataManager()
        ...
        variance_ratio = (np.array(between_class_variances)
                          / np.array(within_class_variances))
        print("\nFor dataset: {} with {}th layer having biggest ratio, "
              "variance ratio: {}\n".format(dataset,
                                            np.argmax(variance_ratio),
                                            variance_ratio))
        plt.plot(layers, variance_ratio, label=dataset)
...
def run_step3(train_sets, train_set_sizes, model_family,
    ...
    """from truth_directions_main import compute_orthonormal_vectors
       Orthonormalise t_g and t_p. As mentioned in the introduction, 
    the authors demonstrate the existence of two truth directions in 
    the activation space: the general truth direction t_G and the 
    polarity-sensitive truth direction t_P. In Figure 1 the authors 
    visualise the projections of the activations a_ij onto the 2D 
    subspace spanned by our estimates of the vectors t_G and t_P. In 
    this visualization of the subspace, we choose the orthonormalized 
    versions of tG and tP as its basis. We discuss the reasons for 
    this choice of basis for the 2D subspace in Appendix B.
...
def main():
    # Hyperparameters
    model_family = 'Llama2'  # 'Llama2', 'Llama3', or 'Mistral3'
    model_size = '13B'  # '7B', '8B', '13B'
    ...
    if run_step["step1"]:
        ...
        for model_type in ['chat']:
            print(model_type)
            run_step1(model_family=model_family, model_size=model_size,
                      model_type=model_type)
    ...
    if run_step["step4"]:
        """
           ...
           from train_sets = ["cities", "neg_cities", 
              ...
              "element_symb", "neg_element_symb", 
              "facts", "neg_facts"]
           Plot the Figure 4: The fraction of variance in the centered and 
        averaged activations ˜µ+i, ˜µ−i explained by the Principal Components
        (PCs). Only the first 10 PCs are shown.
           To illustrate that When applying PCA to affirmative statements 
        only (top left), the first PC explains approximately 60% of the variance 
        in the centered and averaged activations, with subsequent PCs contributing 
        significantly less, indicative of a one-dimensional affirmative truth 
        direction. Including both affirmative and negated statements (top center)
        reveals a two-dimensional truth subspace, where the first two PCs account 
        for more than 60% of the variance in the preprocessed activations.
        Just two principal components sufficiently capture the truth-related 
        variance, suggesting only two truth dimensions.
           The authors also verified that these two PCs indeed approximately 
        correspond to t_G and t_P by computing the cosine similarities between 
        the first PC and t_G and between the second PC and tP, measuring cosine 
    ...
    if run_step["step6"]:
        """
           Plot part of the Figure 5: difference?
        """
