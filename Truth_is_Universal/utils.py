...
def collect_acts(dataset_name, model_family, model_size,
                 model_type, layer, center=True, scale=False,
                 device='cpu'):
    """
       Collects activations from a dataset of statements, returns as a
    tensor of shape [n_activations, activation_dimension].
    """
    # print("\n=>=> ROOT is {}\n".format(ROOT))
    directory = os.path.join(ROOT, 'acts', model_family, model_size, model_type, dataset_name)
    # print("\n=>=> directory is {}\n".format(directory))
    activation_files_path = os.path.join(directory, f'layer_{layer}_*.pt')
    activation_files = glob(activation_files_path)
    # print("\n=>=> activation files are {}\n".format(activation_files))
    acts = [torch.load(os.path.join(directory, f'layer_{layer}_{i}.pt'), map_location=device)
...
def cat_data(d):
    """
       Given a dict of datasets (possible recursively nested),
    returns the concatenated activations and labels.
    """
    all_acts, all_labels = [], []
    for dataset in d:
        if isinstance(d[dataset], dict):
            ...
            all_acts.append(acts), all_labels.append(labels)
    try:
        acts, labels = torch.cat(all_acts, dim=0), torch.cat(all_labels, dim=0)
    except:
        raise Exception("No activation vectors could be found for this "
                        "dataset. Please generate them first "
                        "using generate_acts.")
    return acts, labels
...
class DataManager:
    """
       Class for storing activations and labels
    from datasets of statements.
    """
    def __init__(self):
        self.data = {
            'train': {},
    ...
    def add_dataset(self, dataset_name, model_family, model_size,
                    model_type, layer, label='label', split=None,
                    seed=None, center=True, scale=False, device='cpu'):
        """
           Add a dataset to the DataManager.
        label : which column of the csv file to use as the labels.
           If split is not None, gives the train/val split proportion.
        ...
        if split is not None:
            ...
            train = torch.randperm(len(df)) < int(split * len(df))
            val = ~train
    ...
    def get(self, datasets):
        """
           Output the concatenated activations and labels for
        the specified datasets.
           datasets : can be 'all', 'train', 'val', a list of dataset names,
        or a single dataset name.
        ...
        if datasets == 'all':
            ...
            raise ValueError(f"datasets must be 'all', 'train', 'val', "
                             f"a list of dataset names, or a single "
                             f"dataset name, not {datasets}")
...
def dataset_sizes(datasets):
    """
       Computes the size of each dataset, i.e.
    the number of statements.
    - Input: array of strings that are the names of the datasets
    - Output: dictionary, keys are the dataset names and
    values the number of statements
    """
    dataset_sizes_dict = {}
    for dataset in datasets:
...
def collect_training_data(dataset_names, train_set_sizes, model_family,
                          model_size, model_type, layer, **kwargs):
    """
       Takes as input the names of datasets in the format
    [affirmative_dataset1, negated_dataset1, affirmative_dataset2,
    negated_dataset2, ...] and returns a balanced training dataset of
    (centered activations, activations, labels and polarities).
    ...
    """
    (all_acts_centered, all_acts,
     all_labels, all_polarities) = [], [], [], []
    
    for dataset_name in dataset_names:
        dm = DataManager()
        dm.add_dataset(dataset_name, model_family,
        ...
        ## Create a tensor of specified shape filled with a specified value
        polarities = torch.full((labels.shape[0],), polarity)

        ## Balance the training dataset by including an equal
        # number of activations from each dataset.
        # Choose the same subset of statements for affirmative
        # and negated version of the dataset.
        if 'neg_' not in dataset_name:
            rand_subset = np.random.choice(acts.shape[0],
                                           min(train_set_sizes.values()),
                                           replace=False)
        
        all_acts_centered.append(acts[rand_subset, :] -
                                 torch.mean(acts[rand_subset, :], dim=0))
        all_acts.append(acts[rand_subset, :])
        all_labels.append(labels[rand_subset])
        all_polarities.append(polarities[rand_subset])

    return map(torch.cat, (all_acts_centered, all_acts,
                           all_labels, all_polarities))
    
def compute_statistics(results):
    stats = {}
    for key in results:
        means = {dataset: np.mean(values)
                 for dataset, values in results[key].items()}
        stds = {dataset: np.std(values)
                for dataset, values in results[key].items()}
...
def compute_average_accuracies(results, num_iter):
    ...
    for probe_type in results:
        ...
        for i in range(num_iter):
            ...
            iteration_means = [results[probe_type][dataset][i]
                               for dataset in results[probe_type]]
